...
jQuery.jGlideMenu = {
    ...
    initialize: function (o) {
        ...
        return this.each(function () {
            ...
            var s = {
                ...
                tileSource: 'myTiles',
                URLParams: {},
                closeLinkMarkUp: '../application/assets/img/close.png',
                menuShowFx: 'fadeIn',
                menuHideFx: 'fadeOut',
                easeFx: 'linear',
                loadImage: '../application/assets/img/ajax.gif',
                initialTile: 'tile_001',
                alertOnError: false,
                captureLinks: true,
                loadImageStyle: {'position': 'absolute', 'bottom': '10px', 'left': '10px', 'z-index': '999'},
                imagePath: '../application/assets/img/',
                demoMode: false
            };
    ...
    loadTile: function (u, p) {
        ...
        jQuery('#' + ptr.id + ' div.jGM_content a').bind('click', function () {
            ...
            if (target.substr(0, 1) == '#' && target.substr(0, 4) != '#geo') {
                ...
                var dest = array[0];
                var target_ligne = array[1].substr(3);

                /*Function 2: display the line and stations of the subway on the map
                 *
                 */
                // Determine if current line is visible
                var active_line = d3.select("#l_" + target_ligne).classed("active");
                var color_line = d3.select("#l_" + target_ligne).style("stroke");
                var active_station = d3.select("#l_" + target_ligne).classed("active");
                var newOpacity = active_line ? 0 : 1;
                // Hide or show the elements
                d3.selectAll("#l_" + target_ligne).style("opacity", newOpacity);
                d3.selectAll("path[id^=s]")
                    .filter(function (d, i) {
                        var str = this.id;
                        return (str.match("^s" + target_ligne + "-|^s" + target_ligne + "$|-" + target_ligne + "-|-" + target_ligne + "$")) != null;
                    })
                ...
                d3.selectAll("#l_" + target_ligne).classed("active", active_line = newOpacity);
                d3.selectAll("path[id^=s]")
                    .filter(function (d, i) {
                        var str = this.id;
                        return (str.match("^s" + target_ligne + "-|^s" + target_ligne + "$|-" + target_ligne + "-|-" + target_ligne + "$")) != null;
                    ...
                    .classed("active", active_station = newOpacity);
                ...
                lat = array[0];
                longt = array[1];
                map.setView(new L.LatLng(lat, longt), 19);
            } else {
        ...
        if (jQuery.jGlideMenu.tileCount > 1) {
            ...
            jQuery('#' + ptr.id + ' div.jGM_back').bind('click', function () {
                var newOpacity = 0;
                // Hide the elements
                d3.selectAll("path[id^=l_]").style("opacity", newOpacity);
                d3.selectAll("path[id^=l_]").classed("active", newOpacity);
                d3.selectAll("path[id^=s]").style("opacity", newOpacity);
                d3.selectAll("path[id^=s]").classed("active", newOpacity);
                //zoom out
                map.setView(new L.LatLng(48.850705, 2.357168), 12);
    ...
    buildTile: function (t, d, l) {
        ...
        for (var i = 0; i < l.length; i++) {
            ...
            if (l[i][0].substr(0, 3) != "geo") {
                template += '<a href="' + hash + l[i][0] + '"' + type + '>' + '<img src="../application/assets/img/metro/L_M.png" width="13px" height="13px"/>' +
                    '<img src="../application/assets/img/metro/' + l[i][0] + '.png" width="13px" height="13px"/> ' + l[i][1] + '</a>';
            } else {
