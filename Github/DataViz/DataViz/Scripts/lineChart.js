//Thank you to Reference: Peter Cook @prcweb prcweb.co.uk

var urlParams = getUrlParams();
fileName = urlParams[0];
...
var chart = {
  ...
  perspectiveOffsetX: 6,
  perspectiveOffsetY: 12,
  ...
  init: function(Keyslength) {
	...
	//get the highest value 
  	...
  	for (var k=0; k<12;k++) 
  	{
  		var tempMax = temp[k].max;
  		var tempMin = temp[k].min;
		if(tempMax > highestValue)
		{
			highestValue = tempMax;
		} 
		if(tempMin < lowestValue)
		{
			lowestValue = tempMin;
		} 
	}

	var averageSpac = (highestValue - lowestValue)/10;
	this.data = {data: dataset.reverse() , extent: [0, highestValue]};

  	d3.select('body').style('height', this.bodyHeight + 'px');
  	this.windowHeight = $(window).height();
  	this.scrollScale = d3.scale.linear().domain([0, this.bodyHeight - this.windowHeight * 0.99]).range([0, Keyslength-1]).clamp(true);
    ...
    this.sortFunction.min = function(a, b) {return d3.ascending(a.min, b.min);}
  	...
  	this.initChart(lowestValue,averageSpac);
  	this.initEvents();
    this.initMenu();
  },
  ...
  updateInfo: function() {
    ...
    html += '<p>Tourism peak month: ' + d.max + ' pictures</p>';
    html += '<p>Tourism lowest month: ' + d.min + ' pictures</p>';
    html += '<p>Normal tourism month: ' + d.mean.toFixed(1) + 'pictures</p>';
  ...
  initChart: function(lowestValue,averageSpac){
  	var that = this;
    ...
    years
  	...
  	this.renderAxes(lowestValue,averageSpac);
  	},

  renderAxes: function(lowestValue,averageSpac) {
    ...
    var yAxis = d3.svg.axis()
      .scale(this.yScale)
      .orient('left')
      .tickValues([lowestValue, lowestValue + averageSpac, 2*averageSpac+lowestValue ,  3*averageSpac+lowestValue,  4*averageSpac+lowestValue,  5*averageSpac+lowestValue,  6*averageSpac+lowestValue,  7*averageSpac+lowestValue, 8*averageSpac+lowestValue, 9*averageSpac+lowestValue, 10*averageSpac+lowestValue]);
...
d3.json(filePath, function(temperatureData) {

	chart.data = temperatureData;
	var Keyslength = Object.keys(chart.data).length;
	chart.init(Keyslength);
